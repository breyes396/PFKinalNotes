...
public class LoginView implements Serializable {
    ...
    private final CursoService cursoService; // nuevo para obtener el nombre del curso

    public LoginView(UsuarioService usuarioService, AuthSession authSession, DocenteService docenteService, EstudianteService estudianteService, CursoService cursoService) {
            ...
            switch (usuario.role()) {
                case TEACHER -> {
                    ...
                    if (docente != null) {
                        ...
                        if(docente.codigo_curso() != null){
                            try {
                                CursoDto curso = cursoService.obtenerCursoPorCodigo(docente.codigo_curso());
                                if(curso != null){
                                    authSession.setDocenteCursoNombre(curso.name());
                                    authSession.setDocenteCursoGrado(curso.degree());
                                } else {
                                    authSession.setDocenteCursoNombre("Curso #"+docente.codigo_curso());
                                }
                            } catch (Exception ex){
                                authSession.setDocenteCursoNombre("Curso #"+docente.codigo_curso());
                            }
                ...
                case STUDENT -> {
                    EstudianteDto est = estudianteService.obtenerEstudiantePorCodigoUsuario(usuario.codigo());
                    if(est != null){
                        authSession.setEstudianteCodigo(est.codigo());
                    }
                }
                default -> {}
            }
